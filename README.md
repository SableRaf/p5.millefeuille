...
### `createLayerSystem(settings)`

Creates a new LayerSystem instance (available in global mode or as `sketch.createLayerSystem()` in instance mode).

**Parameters:**
- `settings` (object, optional) - Layer system settings
  - `uriBaseName` (string) - Base name for URI parameter prefix (default: `"layers"`)
...
// Custom URI base name
const layers = createLayerSystem({ uriBaseName: 'myLayers' });
...
Layer properties can be controlled via URL query parameters. URI state is automatically applied on the first call to `render()`, so all layers must be created before then. The base name defaults to `layers` and can be customized via `createLayerSystem({ uriBaseName: 'custom' })`.

**Parameters:**
- `{baseName}.show` - Comma-separated list of layer names to show (all others are hidden)
- `{baseName}.{layerName}.opacity` - Layer opacity (0-1)
- `{baseName}.{layerName}.visible` - Layer visibility (`true`/`false` or `1`/`0`)
- `{baseName}.{layerName}.blendMode` - Layer blend mode (e.g., `MULTIPLY`, `SCREEN` — see [Blend Modes](#blend-modes))

The `show` parameter is applied first, then per-layer properties override it. Unknown layer names and invalid values are silently ignored.

**Example:**
```
?layers.show=bg,fg&layers.bg.opacity=0.5&layers.fg.visible=false&layers.bg.blendMode=SCREEN
```
...
`dist/` is generated by CI and not checked in. `npm run examples` now rebuilds `dist/` before starting the local server, so the local demos always use the latest local `p5.millefeuille` bundle. See [PUBLISHING.md](PUBLISHING.md) for release instructions.
